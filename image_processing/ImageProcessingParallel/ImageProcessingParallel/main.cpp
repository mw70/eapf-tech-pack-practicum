...
void processSubImage( imageData_t *, imageData_t *, unsigned int, unsigned int, unsigned int, unsigned int );
...
// process the buffer
void
processSubImage( imageData_t *input, imageData_t *output, unsigned int startX, unsigned int endX, unsigned int startY, unsigned int endY )
{
    for ( unsigned int i = startY; i < endY; i++ )
    {
        for ( unsigned int j = startX; j < endX; j++ )
        {
            float inRGB[3];
            float outRGB[3];
            unsigned int position = ((i * output->width) + j)*3;
...
processImage( imageData_t *input )
{
    ...
    group.create_thread(boost::bind(processSubImage, input, output, 0, input->width, 0, input->height/4));
    group.create_thread(boost::bind(processSubImage, input, output, 0, input->width, input->height/4, input->height/2));
    group.create_thread(boost::bind(processSubImage, input, output, 0, input->width, input->height/2, input->height*3/4));
    group.create_thread(boost::bind(processSubImage, input, output, 0, input->width, input->height*3/4, input->height));    
    group.join_all();
...
{
    imageData_t * inputBuffer = readJpegFile("/Users/kgoldsmi/Development/EAPF Tech Pack 2/ImageProcessingParallel/test.jpg");
    ...
    if ( writeJpegFile( "/Users/kgoldsmi/Development/EAPF Tech Pack 2/ImageProcessingParallel/test2.jpg", outputBuffer) < 0 )
    {
